...
def train(files_list, session_names = [], net_params=None, n_pop=15, hemi='right', max_time_index =275, lr=5e-4, max_epochs = 300000, test_every = 50,  optimize_every = 1, info = ''):
    thres_trial_av = 3
    #coeficient losses
    c_coding_av = .03 #rec neurons
    ...
    print('Holi', default_parameters['n_neurons'])
    #trial average
    ...
    while epoch<max_epochs:
        ...
        results, cov = net.forward(mean_licks, single_trial_licks[session_names[0]], trial_types)
        ...
        for s in range(len(session_names)):
            ...
            single_trial_neural_activity = single_trials_activity[session_names[s]][:,:,indexes_neurons_st[s]]
            total_time_average += ListLossesTime[s](single_trials_activity[session_names[s]][:,:,indexes_neurons_st[s]], results['rates_trials'][:,:,indexes_neurons_st[s]])
        ...
        for s in range(len(session_names)):
            #trial indexes to compute
            single_trial_neural_activity = single_trials_activity[session_names[s]][:,:,indexes_neurons_st[s]]
            total_coding_average += ListLossesCoding[s](net, single_trial_neural_activity, results['latents_trials']) # Note: problem here with stitching
        ...
        print(f'Epoch {epoch}'+f'| Total Loss {mean_total_loss}' +f' | Trial Av. = {mean_trial}'+f' | Epoch Av. = {mean_time}'+f' | Coding Av. = {mean_coding}'+ f'| Sel. = {sel_loss}'+f' | Orth. = {np.mean(weights_loss)}')
