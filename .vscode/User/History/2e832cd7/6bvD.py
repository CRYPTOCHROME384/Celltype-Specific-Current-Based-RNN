...
def train(files_list, n_pop=15, net_params=None, lr=5e-4, max_epochs = 500000, total_trials = 300, info = ''):
    #loading data    
    ALM_hemi= load_me_data(files_list, default_parameters, max_time_index=235)
    total_sessions = len(files_list)
    d_ind = int(total_trials/total_sessions)
    trial_indexes = [range(10, d_ind + 10) for x in ALM_hemi['indexes']]
    trial_average, trial_types = toch_version_ratates_trial_types(ALM_hemi, default_parameters, total_trials)
    default_parameters['n_latents'] = n_pop
    default_parameters['n_neurons'] = ALM_hemi['rates'].shape[2]

    #coeficient losses
    c_neurons_av = .1 #rec neurons
...
def train_trial_average(files_list, net_params=None, n_pop=15, hemi='left', lr=5e-4, max_epochs = 500000, c_av = 0.7, c_orth = 500, c_sel = 0.5, test_every = 50, optimize_every = 1):
    #loading data    
    ALM_hemi= load_me_data(files_list, default_parameters, hemi=hemi, max_time_index=245)
    trial_average = tch.tensor(ALM_hemi['rates'][0:2,:,:], dtype = tch.float32).to(default_parameters['device']).float()
    print('dimensions', hemi, trial_average.shape)
    default_parameters['n_latents'] = n_pop
    default_parameters['n_neurons'] = ALM_hemi['rates'].shape[2]
    ...
    while epoch<max_epochs:
        #adding initial data
        trial_types = [0, 0] #trainin just trail average
        results, cov = net.forward(trial_average, trial_types)
        ...
        #defining total loss
        loss_factors =  c_orth * total_orth + c_sel * total_selectivity
        total_loss = c_av *  total_trial_average + 0.3 * loss_factors
        total_loss= total_loss/ optimize_every
        ...
        sel_loss = np.mean(losses_sel[max(0, epoch-100):epoch])
        weights_loss = np.mean(losses_weights[max(0, epoch-100):epoch])
        print('Hemi:'+hemi+f' |Epoch {epoch}'+f' | Trial Average = {mean_trial}'+ f'| Selectivity = {sel_loss}'+f' | Orthogonality = {np.mean(weights_loss)}')
        #try:
        #    print('Tf biases grad:', tch.norm(net.biases_alm.grad))
        #    print('Tf coef grad:', tch.norm(net.coef_phi_alm.grad))
        #    print('J rec grad:', tch.norm(net.J_rec.grad))
        #    print('decoder grad:', tch.norm(net.decoder_alm.grad))
        #except:
        #    print('first epoch training')
        
        #optimizing
        optimizer.zero_grad()
        total_loss.backward()
        tch.nn.utils.clip_grad_norm_(net.parameters(),  max_norm=8.5, norm_type=2)
        optimizer.step()
        ...
        if epoch != 0 and epoch %test_every == 0 and mean_loss < min_mean_loss:
            min_mean_loss = min(min_mean_loss, mean_trial)
            print('saving net')
            name_f = path_save+'net_ALM_hemi_'+ hemi +str(n_pop)+ '.pth'
            tch.save(net.state_dict(), name_f)   
        if epoch != 0 and epoch % test_every == 0:
            #Trial average
            name_file = path_save+ 'trial_average_npop_'+str(n_pop)+'_hemi_'+ hemi
            name_loss = 'Loss Reconstructions Neurons Hemi ' + hemi
            pickle.dump(losses_trial_av, open(name_file+'.p', 'wb'))
            loss_plot = losses_trial_av
            plot_loss(epoch, loss_plot, name_loss, tag = name_file)
            # weights and selectivity
            name_file = path_save+'weights_npop_' + str(n_pop) + '_hemi_'+ hemi
            pickle.dump(losses_weights, open(name_file+'.p', 'wb'))
            name_file = path_save+'selectivity_npop_' + str(n_pop) + '_hemi_'+ hemi
            pickle.dump(losses_sel, open(name_file+'.p', 'wb'))
 

def train_trial_average_licks(files_list, net_params=None, n_pop=15, hemi='left', lr=5e-4, max_epochs = 500000, c_av = 0.7, c_orth = 500, c_sel = 0.5, test_every = 50, optimize_every = 1):
    #loading data  
    max_time_index = 275 
    ...
    #first trial
    default_parameters['n_trials'] = 2
    net = SingleTrialRateInputLicks(default_parameters)
    net.device = default_parameters['device']
    net.is_trial_average = True
    if net_params is not None:
        try:
    ...
    while epoch<max_epochs:
        #adding initial data
        trial_types = [0, 0] #trainin just trail average
        results, cov = net.forward(mean_licks, trial_types)
