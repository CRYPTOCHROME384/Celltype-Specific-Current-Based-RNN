...
class LossAverageTime:
    ...
    def compute_time_averages(self, spike_activity, delay_time = 1.2):
        ...
        average_sample = tch.sum((t_sample_s<=spike_times) * (spike_times<t_sample_e))/T_sample
        average_delay1 = tch.sum((t_sample_e<=spike_times) * (spike_times<t_delay_e1))/T_delay1
        average_delay2 = tch.sum((t_delay_e1<=spike_times) * (spike_times<t_delay_e2))/T_delay2
        average_response1 = tch.sum((t_delay_e2<=spike_times) * (spike_times<t_response1))/T_response1 
