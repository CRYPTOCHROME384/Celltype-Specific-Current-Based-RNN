...
class SelectivitySingleTrialLoss:
    ...
    def __call__(self, net, rates_data, rates_model):
        #sample vectors
        net_sample_alm = net.decoder_alm[self.mode_sample, self.indexes]
        #delay vectors
        net_delay_alm = net.decoder_alm[self.mode_delay, self.indexes]
        #choice vectors
        net_choice_alm = net.decoder_alm[self.mode_choice, self.indexes]
        proj_sample_data, proj_delay_data, proj_choice_data = self.project_data(rates_data)
        #projections model
        proj_sample_model =(rates_model @ net_sample_alm.T) / float(len(self.indexes))
        proj_delay_model = rates_model @ net_delay_alm.T / float(len(self.indexes))
        proj_choice_model = rates_model @ net_choice_alm.T / float(len(self.indexes))

        loss_sample = tch.mean(tch.square(proj_sample_data - proj_sample_model))
        loss_delay = tch.mean(tch.square(proj_delay_data - proj_delay_model))
        loss_choice = tch.mean(tch.square(proj_choice_data - proj_choice_model))
    ...
    def project_data(self, rates_data):
        #losses
        proj_sample_data = (rates_data @ self.sample_alm[self.indexes]) / float(len(self.indexes))
        proj_delay_data = rates_data @ self.delay_alm[self.indexes] / float(len(self.indexes))
        proj_choice_data = rates_data @ self.choice_alm[self.indexes] / float(len(self.indexes))
    ...
    def gram_schmidt(self, rates):
        ...
        self._delay_vectors(rates)
        self._choice_vectors(rates)
        vectors = [self.sample_alm, self.delay_alm, self.choice_alm]
        orthogonal_vectors = []
        for v in vectors:
            u = v.clone()
            for w in orthogonal_vectors:
...
class LossAverageTime:
    ...
    def __call__(self, rates_data, rates_model):
        ...
        av_time_model = self.compute_time_averages(rates_model)
        av_time_data = self.compute_time_averages(rates_data)
        #calculating losses
        total_loss = tch.mean(tch.square(av_time_data - av_time_model))
        return total_loss

    def compute_time_averages(self, rates):
        average_sample = tch.sum(rates[:, self.ind_sample_s:self.ind_sample_e, :], axis = 1) * (self.dt/0.65) #spikes per s
        average_delay1 = tch.sum(rates[:, self.ind_sample_e:self.ind_delay_e1, :], axis = 1) * (self.dt/.6) #spikes per s
        average_delay2 = tch.sum(rates[:, self.ind_delay_e1:self.ind_delay_e2,:], axis = 1) * (self.dt/.6) #spikes per s
        average_response = tch.sum(rates[:, self.ind_delay_e2:self.ind_response, :], axis = 1) * (self.dt/self.t_after_go) #spikes per s
