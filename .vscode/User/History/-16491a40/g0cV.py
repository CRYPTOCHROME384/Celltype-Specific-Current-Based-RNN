...
class LossAverageTime:
    ...
    def __call__(self, rates_data, rates_model):
        ...
        #calculating losses
        total_loss = tch.mean(tch.square(av_time_model - av_time_data))
        return total_loss

    def compute_time_averages(self, spike_activity, delay_time = 1.2):
        ...
        average_sample = tch.mean((t_sample_s<=spike_activity) * (spike_activity<t_sample_e))/T_sample
        average_delay1 = tch.sum((t_sample_e<=spike_activity) * (spike_activity<t_delay_e1))/T_delay1
        average_delay2 = tch.sum((t_delay_e1<=spike_activity) * (spike_activity<t_delay_e2))/T_delay2
        average_response1 = tch.sum((t_delay_e2<=spike_activity) * (spike_activity<t_response1))/T_response1 
        average_time = tch.stack([average_sample, average_delay1,average_delay2, average_response1])
        return av_time
