...
class SingleTrialLicks(tch.nn.Module):
    def __init__(self, params_dict, indexes_neurons_st):
        ...
        self.single_trial_mask = tch.zeros((self.bs, single_trial_licks.shape[1], input_licks.shape[1]), device=self.device)
    ...
    def _input_single_trial_licks(self, t, single_trial_licks):
        ...
        i=int(t/self.dt)
        if  self.t_go + 0.1<t:
            input_current_single_trial  = tch.einsum('bj,bs,bsj->bsj', input_licks, single_trial_licks[:,:, i], single_trial_mask)
        else:
