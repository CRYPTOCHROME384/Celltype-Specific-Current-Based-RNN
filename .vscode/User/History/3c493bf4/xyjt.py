...
class SingleTrialRate(tch.nn.Module):
    def __init__(self, params_dict):
        super(SingleTrialRate, self).__init__()
    ...
    def update_alm_mean(self, r_rec, t):
        """"Update alm mean"""
        #current input
        ...
        #current task
        current_task = self._input_sample(t) @ self.decoder_alm + self._input_go(t) @ self.decoder_alm
        #current recurrent
        overlap = r_rec @ self.decoder_alm.T
        current_rec = overlap @ (self.J_rec.T @ self.decoder_alm) 
        #total current
        current = current_rec + current_inp + current_test + current_task
    ...
    def update_alm_trials(self, r_rec, t, trial_type):
        ...
        current_rec = overlap @ (self.J_rec.T @ self.decoder_alm) 
        #total current
        current = current_rec + current_inp + current_test + current_task + current_trials 
        fr_phi =  self.coef_phi_alm * (self.phi(current) + tch.ones_like(current))/2.
        #euler update
        r_rec = r_rec + self.dt * (-r_rec + fr_phi) / self.tau_alm
        return overlap, current, r_rec
    ...
    def forward(self, rates, trial_types):
        y_alm = rates
        bs = y_alm.shape[0]
        n_times = y_alm.shape[1]
        #varibales alm left 
        ...
        for i in range(n_times):
            ...
            m_alm, h_alm, fr_alm = self.update_alm_mean(fr_alm, t)
            if self.is_trial_average==False:
                #update acording single trials
                m_trials, h_trials, fr_trials = self.update_alm_trials(fr_trials, t, trial_types)
            t += self.dt 
...
class SingleTrialRateInputLicks(tch.nn.Module):
    def __init__(self, params_dict):
        super(SingleTrialRateInputLicks, self).__init__()
        self.device = tch.device(params_dict['device'])
        self.dt = params_dict['dt']
        self.input_noise = params_dict['input_noise']
        self.t_min_input = params_dict['t_min_input']
    ...
    def _input_licks(self, t, mean_licks):
        ...
        i=int(t/self.dt)
        if  self.t_go + 0.1<t:
            input_current  = tch.einsum('bj,bj->bj', input_licks, mean_licks[:,i,:])
        else:
            input_current = tch.zeros((self.bs, self.n_neurons_alm)).to(self.device)
        return input_current
    ...
    def update_alm_mean(self, r_rec, mean_licks, t):
        ...
        current_licks = self._input_licks(t, mean_licks) 
        #current recurrent
        overlap = r_rec @ self.decoder_alm.T
        current_rec = overlap @ (self.J_rec.T @ self.decoder_alm) 
        #total current
    ...
    def forward(self, mean_licks, trial_types):
