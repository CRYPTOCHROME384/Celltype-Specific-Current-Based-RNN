...
class SingleTrialLicks(tch.nn.Module):
    ...
    def _input_single_trial_licks(self, t, mean_licks, single_trial_licks):
        """Input current corresponding
        to the left and right licks,
        motor propioceptive inputs
        """
        ...
        if  self.t_go + 0.1<t:
            ...
            input_current_mean = tch.zeros((self.bs, self.n_neurons_alm)).to(self.device)
            input_current_single_trial = tch.zeros((self.bs,  single_trial_licks.shape[1], self.n_neurons_alm)).to(self.device)
        return input_current_mean, input_current_single_trial 
    ...
    def update_alm_mean(self, r_rec, mean_licks, t):
        ...
        current_licks, _ = self._input_licks(t, mean_licks) 
        #current recurrent
