...
def licks_inputs(list_sessions, max_time_index = 350, ind_start=50):
    ...
    for file in list_sessions:
        data_bh = pickle.load(open(path_dandi+ file.rstrip('.p') + '_behavior.p', 'rb'))
        inds_good_trials = data_bh['inds_good_trials']
        print(inds_good_trials.shape, data_bh['rates_left_licks'].shape, data_bh['rates_left_licks'].shape)
        left_licks = data_bh['rates_left_licks'][inds_good_trials, ind_start:max_time_index + ind_start]
        right_licks = data_bh['rates_left_licks'][inds_good_trials, ind_start:max_time_index + ind_start]
        sess_licks = np.concatenate((left_licks,right_licks)) 
